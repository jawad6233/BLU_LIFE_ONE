...
#define TPD_I2C_SLAVE_ADDR (0x40 >> 1)
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!list_empty(&rmi->support_fn_list)) {
		list_for_each_entry_safe(fhandler, next_list_entry, &rmi->support_fn_list, 
link) {
			if (fhandler->fn_number == SYNAPTICS_RMI4_F1A) {
				ii = fhandler->intr_reg_num;
...
{
	CTP_DBG("tpd_i2c_read_data   addr == 0x%x,   length == %d\n",addr,length);
	int retval=0;
	u8 retry = 0;
	u8 *pData = data;
	int tmp_addr = addr;
...
{	
	...
	//clear interrupt bit
	CTP_DBG("Magnum f01.data_base == 0x%x   \n",ts->f01.data_base + 1);
...
//	if (status & 0x20) { /* key */
	if (status & ts->fn1a_mask) { /* key */
		...
		u32 button_addr = 0x0200;
		
		retval = tpd_i2c_read_data(ts->client, button_addr, &button, 1);
		CTP_DBG("======>hx read button regisger 0x%04x ,button_addr == 0x%04x, button == 	0x%x!!!!\n",ts->f1a.data_base,button_addr,button);
		if (button)
		{
			...
			{
			  if((button & (0x01 << i))==1){
			  	CTP_DBG("======>button  1111111      !!!!\n");
			  	input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
			  	//tpd_down(100,850,50);
			  	id = i;
			  }
			  if((button & (0x01 << i))==2){
			  	CTP_DBG("======>button   2222222    !!!!\n");
			  	input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
			  	//tpd_down(250,850,50);
			  	id = i;
			  }
			  if((button & (0x01 << i))==4){
			  	CTP_DBG("======>button 4444444    !!!!\n");
			  	input_report_key(tpd->dev, touch_key_array[i], button & (0x01 << i));
			  	//tpd_down(400,850,50);
			  	id = i;
			  }
...
static int tpd_local_init(void)
{
	...
	printk(" s3202 Synaptics I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
